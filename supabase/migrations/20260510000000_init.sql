...
drop policy if exists "tenant_isolation" on price_observations;
...
create policy "tenant_isolation" on products
  for all using (tenant_id = auth.uid()) with check (tenant_id = auth.uid());
drop policy if exists "tenant_isolation" on product_links;
create policy "tenant_isolation" on product_links
  for all using (tenant_id = auth.uid()) with check (tenant_id = auth.uid());
drop policy if exists "tenant_isolation" on produce_catalog;
create policy "tenant_isolation" on produce_catalog
  for all using (tenant_id = auth.uid()) with check (tenant_id = auth.uid());
drop policy if exists "tenant_isolation" on produce_map;
create policy "tenant_isolation" on produce_map
  for all using (tenant_id = auth.uid()) with check (tenant_id = auth.uid());
drop policy if exists "tenant_isolation" on vendor_rules;
create policy "tenant_isolation" on vendor_rules
  for all using (tenant_id = auth.uid()) with check (tenant_id = auth.uid());
drop policy if exists "tenant_isolation" on product_meta;
