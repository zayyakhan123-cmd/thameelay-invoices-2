...
// the session URL. If the user has no Stripe customer ID yet, one is created
...
serve(async (req) => {
  ...
  try {
    ...
    return json({ error: "Invalid request body" }, origin, { status: 400 });
  }

  if (!price_id) {
    return json({ error: "price_id is required" }, origin, { status: 400 });
  }
